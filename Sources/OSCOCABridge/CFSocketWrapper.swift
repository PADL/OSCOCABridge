...
final class CFSocketWrapper: @unchecked
Sendable, CustomStringConvertible, Hashable {
  static func == (lhs: CFSocketWrapper, rhs: CFSocketWrapper) -> Bool {
  ...
  private enum Element {
    ...
    case nativeHandle(CFSocketWrapper)
    ...
    var nativeHandle: CFSocketWrapper {
      switch self {
  ...
  var acceptedSockets: AnyAsyncSequence<CFSocketWrapper> {
    channel.map(\.nativeHandle).eraseToAnyAsyncSequence()
  }

  var receivedMessages: AnyAsyncSequence<CFSocket.Message> {
  ...
  fileprivate nonisolated func acceptCallBack(
    ...
    nativeHandle!.withMemoryRebound(to: CFSocketNativeHandle.self, capacity: 1) { nativeHandle in
      ...
      Task {
        let socket = try await CFSocketWrapper(nativeHandle: nativeHandle)
  ...
  struct Options: OptionSet, Sendable {
    ...
    static let server = Options(rawValue: 1 << 0)
  }
  ...
  func send(data: Data, to address: (any SocketAddress)? = nil) throws {
